...
That copies `init.lua` into `<game>/bin/x64/plugins/cyber_engine_tweaks/mods/g13-hud/`, fetches the pad screen from
its own repository - [logitech-g13-applets](https://github.com/npc-nathan/logitech-g13-applets) - into
`~/.config/g13/applets/cp2077-hud.json` with its `{GAME}` placeholder replaced by wherever the game actually is, and
adds that applet to `visuals.json` so the pad can walk to it. Nothing outside those places is touched, and re-running
it is how to update. With no network the mod still goes in and the message says how to add the screen by hand.
...
Two other things, both on the driver's side of the pair rather than in the archive:

- **the pad screen** - it is the driver's configuration, not the game mod's, so it lives in
  [logitech-g13-applets](https://github.com/npc-nathan/logitech-g13-applets) with any other extras. `install.sh`
  fetches it and fills in the game path; the driver window's **Menu** tab is what puts it on the pad.
